...
def record_guardian_info(pathlog):
    ...
    for x in f:
        ...
        if ('metadata' in x) and ( 'predict_lane_num' not in x):
            ...
            guardian_real_time = str(utc_date - datetime.timedelta(hours=4))
            data_row[0] = guardian_real_time[:-2] + '00'

            if res['camera_id'] >= 5 and res['camera_id'] < 11:
                if res["camera_id"] != 11 and 'queues' in res['metadata']:
...
def merge_data(data_file_location):
    ...
    return
    # '1012_local.json','17--18.json','18--19.json','19--20.json'
files = ['20--21.json','21--22.json','22--23.json','23--24.json','24--25.json','25--26.json','26--27.json','27--28.json','28--29.json','29--30.json','30--31.json']
